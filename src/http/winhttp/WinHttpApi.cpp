...
WinHttpApi::WinHttpApi(void)
	:hWinHttp_(NULL),
	 ...
	 bUseWinHttp_(TRUE),
	 bInited_(FALSE),
	 m_bEnabledProxy(FALSE)
{
	m_strProxyUrl.clear();
	m_strProxyPassword.clear();
	m_strProxyUserName.clear();
...
HINTERNET WinHttpApi::OpenSession(LPCWSTR userAgent/* = NULL*/)
{
	...
	{
		if (m_bEnabledProxy)
			hSession = (WinHttpApi_.pWinHttpOpen)(userAgent, WINHTTP_ACCESS_TYPE_NAMED_PROXY, m_strProxyUrl.c_str(), WINHTTP_NO_PROXY_BYPASS, NULL/*WINHTTP_FLAG_ASYNC*/);
		else
			hSession = (WinHttpApi_.pWinHttpOpen)(userAgent, WINHTTP_ACCESS_TYPE_NO_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, NULL/*WINHTTP_FLAG_ASYNC*/);
	}
	else
	{
		if (m_bEnabledProxy)
			hSession = (WinINetApi_.pInternetOpen)(userAgent, INTERNET_OPEN_TYPE_PROXY, m_strProxyUrl.c_str(), NULL, INTERNET_FLAG_MAKE_PERSISTENT);
		else
...
HINTERNET WinHttpApi::OpenRequest(HINTERNET hConnect, LPCWSTR verb, LPCWSTR objectName, int scheme)
{
	...
	if (bUseWinHttp_)
	{
		...
		if (m_bEnabledProxy)
		{
			WinHttpApi_.pWinHttpSetOption(hRequest, WINHTTP_OPTION_PROXY_USERNAME, (LPVOID)m_strProxyUserName.c_str(), (DWORD)(m_strProxyUserName.size() + 1));
			WinHttpApi_.pWinHttpSetOption(hRequest, WINHTTP_OPTION_PROXY_PASSWORD, (LPVOID)m_strProxyPassword.c_str(), (DWORD)(m_strProxyPassword.size() + 1));
		}
	...
	else
	{
		...
		if (m_bEnabledProxy)
		{
			WinHttpApi_.pWinHttpSetOption(hRequest, INTERNET_OPTION_PROXY_USERNAME, (LPVOID)m_strProxyUserName.c_str(), (DWORD)(m_strProxyUserName.size() + 1));
			WinHttpApi_.pWinHttpSetOption(hRequest, INTERNET_OPTION_PROXY_PASSWORD, (LPVOID)m_strProxyPassword.c_str(), (DWORD)(m_strProxyPassword.size() + 1));
		}
...
BOOL WinHttpApi::SetProxy(LPCWSTR szProxyUrl, LPCWSTR szProxyUsername, LPCWSTR szPassword)
{
	m_strProxyUrl.assign(szProxyUrl);
	m_strProxyUserName.assign(szProxyUsername);
	m_strProxyPassword.assign(szPassword);

	if (!m_strProxyUrl.empty())
		m_bEnabledProxy = TRUE;
	return TRUE;
}