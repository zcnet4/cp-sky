...
typedef BOOL (WINAPI *tWinHttpQueryOption)(IN HINTERNET hInternet, IN DWORD dwOption, OUT __out_data_source(NETWORK) LPVOID lpBuffer OPTIONAL, IN OUT LPDWORD lpdwBufferLength);
...
public:
	...
	BOOL SetProxy(LPCWSTR szProxyUrl, LPCWSTR szProxyUsername, LPCWSTR szPassword);
public:
...
protected:
	...
	BOOL	bUseWinHttp_;
	BOOL	bInited_;
    BOOL    m_bEnabledProxy;
	std::wstring m_strProxyUrl;
	std::wstring m_strProxyUserName;
	std::wstring m_strProxyPassword;
};
